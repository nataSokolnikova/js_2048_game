...
document.addEventListener('keydown', (e)=>{
    if (game.getStatus() !== 'playing') return; // гра не розпочата — игноруємо клавіші
    switch(e.key){
        case 'ArrowLeft':
            game.moveLeft();
            break;
 ...
 */ class Game {
   ...
   */ constructor(initialState){
        ...
        if (initialState) {
            for(let r = 0; r < this.size; r++)for(let c = 0; c < this.size; c++)this.field[r][c] = initialState[r][c] || 0;
        }
    ...
    addRandomTile() {
        ...
        for(let row = 0; row < this.size; row++){
            for(let col = 0; col < this.size; col++)if (this.field[row][col] === 0) empty.push([
                row,
                col
            ]);
        }
    ...
    checkWin() {
        for (const row of this.field)if (row.includes(2048)) {
            this.status = 'win';
            return true;
        }
        return false;
    ...
    checkLose() {
        for(let row = 0; row < this.size; row++)for(let col = 0; col < this.size; col++){
            if (this.field[row][col] === 0) return false;
            if (col < this.size - 1 && this.field[row][col] === this.field[row][col + 1]) return false;
            if (row < this.size - 1 && this.field[row][col] === this.field[row + 1][col]) return false;
        }
        this.status = 'lose';
        return true;
    }
    moveLeft() {
        ...
        for(let row = 0; row < this.size; row++){
            ...
            if (newRow.join() !== this.field[row].join()) moved = true;
    ...
    rotateLeft() {
        ...
        for(let row = 0; row < this.size; row++)for(let col = 0; col < this.size; col++)newField[this.size - col - 1][row] = this.field[row][col];
        this.field = newField;
    }
    rotateRight() {
        const newField = Array.from({
        ...
        for(let row = 0; row < this.size; row++)for(let col = 0; col < this.size; col++)newField[col][this.size - row - 1] = this.field[row][col];
        this.field = newField;
    }
    /**
   * @returns {number}
